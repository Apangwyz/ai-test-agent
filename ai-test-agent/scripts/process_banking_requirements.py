...
BANKING_INPUT_DIR = os.path.join(os.path.dirname(__file__), '..', 'docs', 'requirements', 'banking')
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), '..', 'docs', 'requirements', 'banking_results')
...
def save_results(project_name, results, output_dir):
    ...
    for key, name in doc_types:
        if key in results:
            ...
            exporter.export_to_markdown(results[key], md_path, key if key != 'structured_data' else 'structured_data')
...
def run_banking_test():
    ...
    if not os.path.exists(BANKING_INPUT_DIR):
        print(f"\n错误: 输入目录不存在 - {BANKING_INPUT_DIR}")
        return
    
    input_files = sorted([f for f in os.listdir(BANKING_INPUT_DIR) if f.endswith('.md')])
    ...
    for filename in input_files:
        file_path = os.path.join(BANKING_INPUT_DIR, filename)
