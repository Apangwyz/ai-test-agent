...
项目使用PostgreSQL作为主数据库，详细的数据库设计文档请参考：

- **数据库设计文档**：`docs/database/DATABASE_DESIGN.md`
- **数据库迁移评估**：`docs/database/MIGRATION_ASSESSMENT.md`（PostgreSQL到MySQL迁移评估）
...
4. **验证数据库初始化**
   ```bash
   # 检查数据库连接
   python -c "from src.auth.database import db; print('数据库连接成功' if db.engine.connect() else '数据库连接失败')"
   ```
...
6. **验证服务启动**
   - 访问 `http://localhost:5000`，应显示系统欢迎页面
   - 检查服务日志，确认无错误信息
